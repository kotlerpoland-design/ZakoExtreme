 ...
 * Tabela dojazdu przed „Dla kogo" (28 % ruchu lokalnego, Białka podświetlona). Telefon nie jest CTA (decyzja 2026-09-10).
  ...
  return (
    <TrackingProvider ctx={{ page_type: "product", product: "buggy", language: locale }}>
      ...
      <main className="flex-1">
        ...
        <Section n={3} title={t("buggy.sections.pricing")} marginLabel="Zakopane · Tatry" tone="fog" className="isolate overflow-hidden">
        ...
        {/* blok C: Maverick — ostatni i najdroższy (górna kotwica cenowa, BUGGY.md §13) */}
        <Section id={MAVERICK_ANCHOR} title={t("buggy.sections.maverick")} tone="fog" className="scroll-mt-header">
          <PricingCards ladder="maverick" locale={locale} layout="stack" included={false} />
        </Section>

        <BookingSection labels={await bookingLabels(t("buggy.sections.booking"))} sectionNumber={4} />

        <Section n={5} title={t("buggy.sections.gallery")} tone="paper">
          <Gallery items={gallery} locale={locale} more={{ href: "/galeria", hash: "buggy", label: t("gallery.seeAll") }} />
        </Section>

        {/* id="dojazd": cel kotwicy z paska miejscowości; Białka podświetlona — jedna czwarta ruchu tej strony (BUGGY.md §8) */}
        <Section id="dojazd" n={6} title={t("buggy.sections.directions")} tone="fog" className="scroll-mt-header">
          <DirectionsTable locale={locale} highlight="bialka" />
        </Section>

        <Section n={7} title={t("buggy.sections.forWhom")} tone="paper">
        ...
        <Section n={8} title={t("buggy.sections.howItWorks")} intro={t("buggy.howItWorksIntro")} tone="fog">
          <Steps locale={locale} />
        </Section>

        <Section n={9} title={t("buggy.sections.faq")} tone="paper">
          <FAQ ids={faqSets.buggy} locale={locale} media={confirmedMedia(media.sections.faq)} />
        </Section>

        {reviews ? (
          <Section n={10} title={t("buggy.sections.reviews")} tone="fog">
            <Reviews limit={3} />
          </Section>
        ) : null}

        <Section n={reviews ? 11 : 10} title={t("home.sections.contact")} tone={reviews ? "paper" : "fog"}>
          <ContactClose />
        </Section>
      </main>
